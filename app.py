...
def apply_threshold(base64_image_data, threshold=128):
    ...
    image_binary = Image.new('L', image_gray.size)
    pixels_in = image_gray.load()
...
def preprocess_image(image_binary):
    ...
    try:
        if ',' in image_binary: # Check if the header is present
            header, encoded = image_binary.split(',', 1)
        else:
            encoded = image_binary # Assume no header
